...
function y=mod9_ALLOMETRY(b_num,ydat,xdat,df,ll)
global A N Z BETA%Z is a matrix containing the partial derivative p() from mod1
T=1/(df);
...
if ll==0%if LL=set all outcome to 1 and reestimate partials based on this
   V1=zeros(N,1);%initializing
   YHAT=zeros(N,1);%initializing
   yones=ones(N,1);%creates an array with all ones for use in making conf. region
   p=zeros(N,b_num); %partial derivative in module 2 is by size= [N,b_num]
   e1=E(yones,xdat,ll); %function call to get error vector or LL also F in the NMRI programs
   for i=1:b_num
      BETA(i)=BETA(i)*1.001;%BETA gets changed temporarily
      e2(:,i)=E(yones,xdat,ll); %e2 is the new error term to be compared to E1  
...
plot(xdat,YHAT,'b:p',xdat,YMIN,'c-',xdat,YMAX,'c-')
disp(['YHAT,         SEYHAT,    YMAX,     YMIN'])
u=[YHAT, SEYHAT, YMAX, YMIN];
u=num2str(u);
disp(u)
